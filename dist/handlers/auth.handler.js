...
async function broadcastRoomUsers(roomId, io) {
    ...
    const data = {
        ...
        payload: {
            ...
            users: all.map(p => {
                ...
                if (room.textEnabledUserIds.has(p.user.id))
                    textAllowed = true;
                if (room.textDisabledUserIds.has(p.user.id))
                    textAllowed = false;
                let attachmentsAllowed = room.settings.attachmentsEnabled;
                if (room.attachmentsEnabledUserIds.has(p.user.id))
                    attachmentsAllowed = true;
                if (room.attachmentsDisabledUserIds.has(p.user.id))
                    attachmentsAllowed = false;
                let drawingAllowed = false;
                if (room.drawingEnabledUserIds.has(p.user.id))
                    drawingAllowed = true;
                return {
                    user_id: p.user.id,
...
// ─── Socket handlers ──────────────────────────────────────────
export function registerAuthSocketHandlers(socket, io) {
    socket.on("join", async ({ roomId, payload }) => {
        ...
        const nameDuplicate = Array.from(room.participants.values()).find(p => p.user.name.toLowerCase() === socket.user.name.toLowerCase() && p.user.id !== socket.userId);
        if (nameDuplicate) {
        ...
        // Duration init
        if (!room.timerStarted && !room.duration && payload?.duration)
        ...
        if (socket.user.isCounsellor)
            room.counsellorSocketId = socket.id;
    ...
    socket.on("disconnect", async (reason) => {
        ...
        if (room.counsellorSocketId === socket.id)
            room.counsellorSocketId = null;
        ...
        if (socket.user?.visitorId) {
            ...
            fetch(`${CFG.MAIN_BACKEND_URL}/api/v1/session/visitor/presence`, {
                ...
                if (!res.ok)
                    log.error(`Presence update failed for visitor ${socket.user?.visitorId}: Status ${res.status}`);
                else
                    log.info(`Presence update success for visitor ${socket.user?.visitorId}`);
