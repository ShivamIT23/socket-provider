...
// ─── Socket handlers ──────────────────────────────────────────
export function registerViewportSocketHandlers(socket, io) {
    // ── Teacher scrolls → students follow ───────────────────────
    socket.on("viewport_update", ({ payload }) => {
    ...
    socket.on("viewport_lock_toggle", ({ payload }) => {
        ...
        if (payload.viewport)
            room.lastViewport = payload.viewport;
...
// ─── REST routes ──────────────────────────────────────────────
export function registerViewportRoutes(app, io) {
