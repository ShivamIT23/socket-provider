...
export function registerDrawingSocketHandlers(socket, io) {
    ...
    socket.on("stroke_draw", ({ payload }) => {
        ...
        if (payload.type === "start") {
            ...
            if (buffer)
                buffer.points.push(payload.point);
        }
    ...
    socket.on("text_update", ({ payload }) => {
        ...
        const existing = room.boardObjects.find((o) => o.type === "text" && o.payload.id === payload.id);
        if (existing) {
            if (payload.position)
                existing.payload.position = payload.position;
            if (payload.text !== undefined)
                existing.payload.text = payload.text;
            if (payload.color)
                existing.payload.color = payload.color;
            if (payload.fontSizeRatio)
                existing.payload.fontSizeRatio = payload.fontSizeRatio;
            room.isDirty = true;
        }
    ...
    socket.on("shape_update", ({ payload }) => {
        ...
        const existing = room.boardObjects.find((o) => o.type === "shape" && o.payload.id === payload.id);
        if (existing) {
            if (payload.position)
                existing.payload.position = payload.position;
            if (payload.widthRatio !== undefined)
                existing.payload.widthRatio = payload.widthRatio;
            if (payload.heightRatio !== undefined)
                existing.payload.heightRatio = payload.heightRatio;
            room.isDirty = true;
        }
    ...
    socket.on("stroke_update", ({ payload }) => {
        ...
        // Persist position/size changes in boardObjects
        const existing = room.boardObjects.find((o) => o.type === "stroke" && o.payload.id === payload.id);
        if (existing) {
            // Store movement offset as position override
            if (payload.position)
                existing.payload.movedPosition = payload.position;
            if (payload.widthRatio !== undefined)
                existing.payload.movedWidthRatio = payload.widthRatio;
            if (payload.heightRatio !== undefined)
                existing.payload.movedHeightRatio = payload.heightRatio;
            room.isDirty = true;
        }
...
// ─── REST routes ──────────────────────────────────────────────
export function registerDrawingRoutes(app) {
...
//# sourceMappingURL=drawing.handler-vcxt.js.map