...
// ─── Socket handlers ──────────────────────────────────────────
export function registerChatSocketHandlers(socket, io) {
    // ── Send message ────────────────────────────────────────────
    socket.on("chat", async ({ payload }) => {
        ...
        if (room.textEnabledUserIds.has(socket.userId))
            textAllowed = true;
        if (room.textDisabledUserIds.has(socket.userId))
            textAllowed = false;
        ...
        if (room.attachmentsEnabledUserIds.has(socket.userId))
            attachmentsAllowed = true;
        if (room.attachmentsDisabledUserIds.has(socket.userId))
...
// ─── REST routes ──────────────────────────────────────────────
export function registerChatRoutes(app, io) {
    // Toggle chat
    app.post("/api/room/:roomId/chat/toggle", (req, res) => {
        const { roomId } = req.params;
        if (!isTeacherAuth(roomId, req.body.userId))
