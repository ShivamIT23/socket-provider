...
function broadcastPageState(roomId: string, io: Server) {
...
export function registerControlsRoutes(app: Application, io: Server) {

  // ── Lock canvas ─────────────────────────────────────────────
  app.post("/api/room/:roomId/lock", (req, res) => {
    const room = requireTeacher(req.params.roomId, req.body.userId, res); if (!room) return;
  ...
  app.post("/api/duration/:roomId", (req, res) => {
    ...
    if (room.timerStarted) return res.status(400).json({ error: "Timer already started" });
  ...
  app.post("/api/room/:roomId/page/delete", (req, res) => {
    ...
    if (room.pages.length <= 1) return res.status(400).json({ error: "Cannot delete last page" });
    room.pages.splice(idx, 1);
    if (room.currentPageId === pageId) room.currentPageId = room.pages[Math.max(0, idx - 1)]!.id;
    room.isDirty = true;
