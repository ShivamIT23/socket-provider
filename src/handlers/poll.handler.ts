...
import type { CustomSocket, Poll, ChatMessage } from "../types.js";
import { ensureRoom, isTeacherSocket } from "../room.js";

export function registerPollSocketHandlers(socket: CustomSocket, io: Server) {
  const broadcastPollState = (roomId: string, room: ReturnType<typeof ensureRoom>) => {
  ...
  socket.on("poll_create", ({ payload }: { payload: { question: string; options: string[] } }) => {
    ...
    if (!room.pollsHistory) room.pollsHistory = [];
  ...
  // ── Vote on poll ────────────────────────────────────────────
  socket.on("poll_vote", ({ payload }: { payload: { optionId: string } }) => {
    ...
    if (room.pollsHistory) {
      ...
      if (histPoll) {
        ...
        if (histOpt && !histOpt.votes.includes(voterId!)) histOpt.votes.push(voterId!);
      }
    }
  ...
  socket.on("poll_end", () => {
    ...
    if (room.currentPoll && room.currentPoll.isActive) {
      ...
      if (room.pollsHistory) {
        const histPoll = room.pollsHistory.find(p => p.id === room.currentPoll?.id);
        if (histPoll) histPoll.isActive = false;
      }
      room.isDirty = true;
      ...
      const chatResultsMsg: ChatMessage = {
        id: crypto.randomUUID(),
  ...
  socket.on("poll_delete", ({ payload }: { payload?: { pollId?: string } }) => {
    if (!socket.roomId || !isTeacherSocket(socket)) return;
    const room = ensureRoom(socket.roomId);
