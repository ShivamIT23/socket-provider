...
import type { CustomSocket, QuizState, ChatMessage } from "../types.js";
import { ensureRoom, isTeacherSocket } from "../room.js";
import crypto from "crypto";

export function registerQuizSocketHandlers(socket: CustomSocket, io: Server) {
  const broadcastQuizState = (roomId: string, room: ReturnType<typeof ensureRoom>) => {
  ...
  socket.on("quiz_create", ({ payload }: { payload: { questions: Array<{ question: string; options: string[]; correctOption: number }> } }) => {
  ...
  socket.on("quiz_submit", ({ payload }: { payload: { answers: Array<{ questionId: string; optionId: string }> } }) => {
  ...
  socket.on("quiz_share_link", ({ payload }: { payload: { shareToken: string; quizTitle: string } }) => {
    if (!socket.roomId || !isTeacherSocket(socket)) return;
    const room = ensureRoom(socket.roomId);
